...
export default {
  ...
  onSocketOpened() {
    ...
    var isFirstTime = true; // active pane item changed for the first time

    this.subscriptions.add(atom.workspace.observeActivePaneItem( (item) => {
      if (isFirstTime) { // do not send activePaneItemChanged for the first time
        isFirstTime = false;
        return;
      }
      let result = this.isProjectFile(item.getPath());
      if (atom.workspace.isTextEditor(item) && result.isProjectFile) {
        let data = {
          a: 'meta',
