...
export default class EventHandler {
  ...
  listen() {
    ...
    disposables.push(this.editor.onDidChangeSelectionRange( (event) => {
      ...
      // cursor moved event
      if (event.newBufferRange.start.isEqual(event.newBufferRange.end)) {
        data.type = 'cursorMoved';
      } else { // selection event
        data.type = 'select';
      }
      data.newRange = event.newBufferRange;
  ...
  listenToServerChanges() {
    this.socket.on('message', (msg) => {
      ...
      switch (data.type) {
        case 'cursorMoved':
          this.onCursorMoved(data.newRange);
          break;
        case 'select':
          this.onSelect(data.newRange);
          break;
        case 'save':
  ...
  onCursorMoved(rangeObj) {
    // type = overlay
    if (typeof this.overlayMarker !== 'undefined') {
      this.overlayMarker.destroy();
    }

    this.overlayMarker = this.editor.markBufferRange(Range.fromObject(rangeObj), {
      'invalidate': 'never'
    });
    let element = document.createElement('div');
    element.textContent = 'Overlay decorations!';
    let style = {
